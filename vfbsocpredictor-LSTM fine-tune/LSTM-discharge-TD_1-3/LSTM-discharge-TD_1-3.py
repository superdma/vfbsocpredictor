...
# ========== Data loading and strict division by Cycle_Index ========== 
data = pd.read_csv('./discharge-2024.08.23.csv', header=0)
save_path = './LSTM-discharge-TD_1-3/'
os.makedirs(save_path, exist_ok=True)

# Confirm data is sorted by Cycle_Index
df = data.sort_values(by='Cycle_Index').reset_index(drop=True)

# Strictly divide according to requirements: training set cycle index < 40, test set cycle index > 40
...
def build_model(units=64, dropout=0.2, num_layers=2, bidirectional=False, lr=0.001):
    """Fix: Change layers parameter name to num_layers to avoid conflict with module name"""
    inputs = keras.Input(shape=(time_step, X_train.shape[1]))
    # Bidirectional LSTM layer
    if bidirectional:
        x = layers.Bidirectional(layers.LSTM(units, return_sequences=True, kernel_regularizer=keras.regularizers.l2(0.01)))(inputs)
    else:
        x = layers.LSTM(units, return_sequences=True, kernel_regularizer=keras.regularizers.l2(0.01))(inputs)
    # Multiple-layer LSTM (Fix: Use num_layers instead of layers)
    for _ in range(num_layers - 1):
        x = layers.LSTM(units, return_sequences=True, kernel_regularizer=keras.regularizers.l2(0.01))(x)
...
def run_hyperparameter_tuning():
                    ...
                    for lr in params['lr']:
                        for batch_size in params['batch_size']:
                            all_combinations.append({
                                'units': units,
                                'dropout': dropout,
                                'num_layers': num_layers,  # Fix: Use num_layers
    ...
    for idx, params in enumerate(all_combinations):
        ...
        # Build model (Fix: Use num_layers parameter)
        model = build_model(
            units=params['units'],
            dropout=params['dropout'],
            num_layers=params['num_layers'],  # Fix: Use num_layers
    ...
    # Select best model (using parameters from dictionary)
    ...
    # Extract all required parameters from the best parameter dictionary
    ...
    # Recreate callbacks with best parameters
    best_callbacks = [
        keras.callbacks.EarlyStopping(
            monitor='loss',
    ...
    # Rebuild and save model with best parameters
...
trues = y_test.flatten()  # y_test is the sequence-processed label, length = len(y_test)

# ========== Fix: Inconsistent array lengths ========== 
# Ensure all arrays have the same length (based on y_test length)
length = len(y_test)  # Base length

# Truncate all arrays to the same length
y_test_cell = y_test_cell[:length]
cycle_index = cycle_index[:length]
trues = trues[:length]  # Although trues is y_test, ensure length consistency
...
# x = list(range(len(cycle_index)))
# plt.plot(x, trues, 'o', label='True SOC', color='#C4161B', markersize=9)
# plt.plot(x, y_test_cell, 'v', label='OCV SOC', color='#FCAA8D', markersize=9)
# plt.plot(x, y_pred_test, '^', label='LSTM Prediction', color='#1E6DB2', markersize=9, alpha=0.8)
# plt.xlim(-500, len(x) + 500)
# plt.ylim(0, 2)
# plt.xlabel('Data Point')
# plt.ylabel('SOC')
# plt.legend(loc='best', frameon=False)
# plt.savefig(os.path.join(save_path, 'final_comparison.png'))
# plt.close()
